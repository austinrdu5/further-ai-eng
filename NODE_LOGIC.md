...
1. `intro`: this node takes in the first query, rephrases the query, and chooses the following:
    ...
    - else, attempt a live contact (function call), and when failed, tells the user that it's a virtual assistant and asks if it wants to leave info for a callback or continue with the conversation
        - if callback, next node is `info_collector` 
        - if continue, next node is `router`

2. `router`: This is an internal routing node that (usually) has no message to the user. 
...
3. `reattempt_live_contact`: 
    ...
    - else, skip the 10 second pause and proceed straight to `info_collector`

4. `info_collector`: This node attempts to populate state.user_info's first_name, last_name, email, phone, and address. 
    - if complete, ask if there's anything else and send to `router`
...
6. `knowledge_base`: this node has a function that pulls information from knowledge.py and populates the prompt. 
